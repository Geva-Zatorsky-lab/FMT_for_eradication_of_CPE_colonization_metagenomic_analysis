...
psonly_no = subset_samples(psfiltered, (Outcome == "Non-responders" | Outcome == "Donor"))
filter <- phyloseq::genefilter_sample(psonly_no, filterfun_sample(function(x) x >= 100), A = 0.2*nsamples(psonly_no))
psonly_no = prune_taxa(filter, psonly_no)
psonly_no_norm = phyloseq_standardize_otu_abundance(psonly_no, method = "total")

#extracting all no samples from functional
subsetColData <- allColData_filtered_samples[grep("Non-responders|Donor", allColData_filtered_samples$Outcome), ]
...
#taxonomy (no donors)
psonly_no_for_DE = subset_samples(psfiltered, (Outcome == "Non-responders"))
filter <- phyloseq::genefilter_sample(psonly_no_for_DE, filterfun_sample(function(x) x >= 100), A = 0.2*nsamples(psonly_no_for_DE))
psonly_no_for_DE = prune_taxa(filter, psonly_no_for_DE)

DES_psonly_no = phyloseq_to_deseq2(psonly_no_for_DE, ~ Timepoint)
DES_psonly_no = DESeq(DES_psonly_no)
...
PS.ord <- ordinate(psonly_no_norm, method = "PCoA",distance =  "bray")

percentVar <- round(100 * PS.ord$values$Relative_eig, digits = 1)

#antibiotic treatment taxonomy
 
plot_ordination(psonly_no_norm, PS.ord, type="samples", color="Antibiotic_treatment_1_month") +
  coord_fixed() +
  #ggtitle("antibiotics 1 month - only no") +
  geom_point(size = 2) +
...
plot_ordination(psonly_no_norm, PS.ord, type="samples", color="Timepoint") +
  coord_fixed() +
  #ggtitle("Timepoint - only no") +
  geom_point(size = 2) +
...
ggplot() +
  ...
  geom_point(data = subset(DEres, abs(log2FoldChange) > 2 & padj < 0.05),
             aes(log2FoldChange, -log10(pvalue)), color = "red", size =2)  
  #geom_text(data = subset(DEres, abs(log2FoldChange) > 4 & padj < 0.05),
          #aes(log2FoldChange, -log10(pvalue)), label = rownames(subset(DEres, abs(log2FoldChange) > 4 & padj < 0.05)), vjust =1)
...
subsetDE2ColData <- allColData_filtered_samples[grep("Non-responders|Donor", allColData_filtered_samples$Outcome), ]
...
head(sigtab)
write.csv(sigtab,"only no/significant modules Non-responders.csv", row.names = TRUE)

#########################################
#     differential analysis - taxonomy  #
#########################################

res = results(DES_psonly_no, contrast=c("Timepoint", "Post-FMT", "Pre-FMT"))
alpha = 0.05
sigtab = res[which(res$padj < alpha), ]
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(psonly_no_for_DE)[rownames(sigtab), ], "matrix"))

#order results by pvalues
sigtab = sigtab[order(sigtab$pvalue),]
head(sigtab)

write.csv(sigtab,"only no/significant OTUs only no timepoints.csv", row.names = TRUE)
